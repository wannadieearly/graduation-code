...
void loop() {
  ...
  if (currentState == DRAWING && currentDotIndex < totalDotsCount) {
    ...
    punchDot();
    currentDotIndex++;
  } else if (currentState == DRAWING && currentDotIndex >= totalDotsCount) {
...
void setupBraillePhrase() {
  ...
  // 床(ch + uang) -> ⠟⠮ -> 011111 111110
  // 前(q + ian)   -> ⠵⠳ -> 011101 110011
  // 明(m + ing)   -> ⠍⠗ -> 001101 101011
  // 月(üe)        -> ⠼   -> 111100
  // 光(g + uang)  -> ⠛⠮ -> 011011 111110
  addBrailleCell(0b011111, current_x); addSpace(current_x, 10); // 字间空格
  addBrailleCell(0b111110, current_x); addSpace(current_x, 10); // 词间空格
...
void addBrailleCell(int pattern, float &offsetX) {
  float localX[] = {0, 0, 0, 10, 10, 10}; // X坐标: 1-3列
  float localY[] = {0, 10, 20, 0, 10, 20}; // Y坐标: 1-3行
  for (int i = 0; i < 6; i++) {
    if ((pattern >> i) & 1) { // 检查pattern的第i位是否为1
      brailleDots[totalDotsCount++] = {offsetX + localX[i], localY[i]};
    }
  }
  offsetX += 10; // cellSpacing = 20mm
}

// 添加空格 (无点)
void addSpace(float &offsetX, float spaceWidth) {
    offsetX += spaceWidth; // wordSpacing or cellSpacing
}
